...
class Asset(models.Model):
    ...
    business_unit = models.ForeignKey('BusinessUnit', null=True, blank=True, verbose_name='所属业务',
                                      on_delete=models.SET_NULL)
    status = models.SmallIntegerField(choices=asset_status, default=0, verbose_name='设备状态')
    manufacturer = models.ForeignKey('Manufacturer', null=True, blank=True, verbose_name='制作商',
                                     on_delete=models.SET_NULL)
    manage_ip = models.GenericIPAddressField(null=True, blank=True, verbose_name='管理IP')
    tags = models.ManyToManyField('Tag', blank=True, verbose_name='标签')
    admin = models.ForeignKey('Contract', null=True, verbose_name='资产管理员', related_name='admin',
                              on_delete=models.SET_NULL)
...
class Server(models.Model):
    ...
    asset = models.OneToOneField('Asset', on_delete=models.CASCADE)  # 非常关键的一对一关联！asset被删除一并删除server
    sub_asset_type = models.SmallIntegerField(choices=sub_asset_type_choice, default=0, verbose_name='服务器类型')
    created_by = models.CharField(choices=create_by_choice, max_length=32, default='auto', verbose_name='录入方式')
    hosted_on = models.ForeignKey('self', related_name='hosted_on_server', blank=True, null=True, verbose_name='宿主机',
                                  on_delete=models.CASCADE)
    model = models.CharField(max_length=128, null=True, blank=True, verbose_name='服务器类型')
    ...
    os_release = models.CharField(max_length=64, blank=True, null=True, verbose_name='发行版本')
...
class SecurityDevice(models.Model):
    ...
    model = models.CharField(max_length=128, default='未知型号', verbose_name='安全设备类型')
...
class StorageDevice(models.Model):
    ...
    asset = models.OneToOneField('Asset', on_delete=models.CASCADE)
    sub_asset_type = models.SmallIntegerField(choices=sub_asset_type_choice, default=0, verbose_name='存储设备')
...
class NetworkDevice(models.Model):
    ...
    asset = models.OneToOneField('Asset', on_delete=models.CASCADE)
    sub_asset_type = models.SmallIntegerField(choices=sub_asset_type_choice, max_length=32, default=0,
                                              verbose_name='网络设备类型')
    vlan_ip = models.GenericIPAddressField(blank=True, null=True, verbose_name='VlanIP')
    intranet_ip = models.GenericIPAddressField(blank=True, null=True, verbose_name='内网IP')
    model = models.CharField(max_length=30, default='未知类型', verbose_name='网络设备类型')
    ...
    device_detail = models.TextField(null=True, blank=True, verbose_name='详情配置')
...
class Software(models.Model):
    ...
    只保存付费购买的软件
    """
    sub_asset_type_device = (
        (0, "操作系统"),
        (1, "办公\\开发软件"),
        (2, "业务软件")
    )

    sub_asset_type = models.SmallIntegerField(choices=sub_asset_type_device, default=0, verbose_name="软件类型")
...
class CPU(models.Model):
    ...
    """
    asset = models.OneToOneField('Asset', on_delete=models.CASCADE)
    cup_model = models.CharField('CPU型号', max_length=128, blank=True, null=True)
    cup_count = models.PositiveSmallIntegerField('物理CPU个数', default=1)
    cpu_core_count = models.PositiveSmallIntegerField('CPU核数', default=1)

    def __str__(self):
        return self.asset.name + "：" + self.cup_model
...
class RAM(models.Model):
    ...
    class Meta:
        ...
        unique_together = ('asset', 'slot')  # 统一资产下的内存，根据插槽的不同，必须唯一
...
class Disk(models.Model):
    ...
    manufacturer = models.CharField('磁盘制作商', max_length=128, blank=True, null=True)
    capacity = models.FloatField('磁盘容量（GB）', max_length=16, choices=disk_interface_type_choice)
...
class NIC(models.Model):
    ...
    mac = models.CharField('MAC地址', max_length=64)  # 虚拟机可能会使用同样的mac地址
...
class BusinessUnit(models.Model):
    ...
    memo = models.CharField('备注', max_length=64, null=True)
...
class Contract(models.Model):
    ...
    memo = models.CharField('备注', max_length=128, null=True)
    price = models.IntegerField('合同金额')
    detail = models.TextField('合同详情', blank=True, null=True)
    start_day = models.DateField('开始时间', blank=True, null=True)
    end_day = models.DateField('结束时间', blank=True, null=True)
...
class NewAssetApprovalZone(models.Model):
    ...
    cpu_model = models.CharField(max_length=128, blank=True, null=True, verbose_name='内存大小')
    cpu_count = models.PositiveSmallIntegerField('cpu物理核心数', blank=True, null=True)
    cpu_core_count = models.PositiveSmallIntegerField('cpu核心数', blank=True, null=True)
    ...
    m_time = models.DateTimeField('修改日期', auto_now_add=True)
