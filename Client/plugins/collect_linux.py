...
def get_os_info():
    ...
    data_dic = {
        'os_distribution': distributor[0].strip() if len(distributor) > 1 else '',
...
def get_cpu_info():
    ...
    :return:
    """
    raw_cmd = 'cat /proc/cupinfo'

    raw_data = {
        'cpu_model': '%s | grep "model name" | head -1 ' % raw_cmd,
        'cpu_count': '%s | grep "processor" | wc -1 ' % raw_cmd,
...
def get_disk_info():
    """
    获取存储版本信息
    本脚本只针对ubuntu使用sda，且只有一块硬盘的情况
    具体查看硬盘信息的命令，请根据实际情况，实际调整
    如果需要查看Raid信息，可以尝试MegaCli工具
    :return:
    """

    raw_data = subprocess.Popen('sudo hdparm -f /dev/sda | grep Model', stdout=subprocess.PIPE, shell=True)
